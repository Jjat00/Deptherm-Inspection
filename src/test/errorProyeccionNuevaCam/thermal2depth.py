...
def startFindCorners(imagesSrc, imagesDst):
    ...
    for which in range(len(imagesDst)):
        ...
        newImage = thermalToDepthImage(imageSrc)
...
def viewImages():
    image = cv2.imread('images/thermal/image43.png')
    newImage = thermalToDepthImage(image)
    newImage = emulatedThermalImage(newImage)
    newImage = setMaxUmbral(newImage, 254.99)
    depthImage = cv2.imread('images/depth/image43.png')
    fusionImage = cv2.addWeighted(
        newImage, 100 / 100, depthImage, 40 / 100, 0)
...
if __name__ == "__main__":
    ...
    #for i in range(3,50):
        ...
        #thermal2depthImage = thermalToDepthImage(thermalImage)
        #depthImage = cv2.imread('images/depth/image%i.png' % i)
        #find = findChessCorners(thermal2depthImage, depthImage)
        #if not find:
